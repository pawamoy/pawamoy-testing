...
def check_api(ctx: Context) -> None:
    ...
    ctx.run(
        griffe_check("pawamoy_testing", search_paths=["src"], color=True),
        title="Checking for API breaking changes",
        command="griffe check -ssrc pawamoy_testing",
        nofail=True,
    )
...
def docs_deploy(ctx: Context) -> None:
    ...
    with material_insiders() as insiders:
        ...
        if "pawamoy-insiders/pawamoy-testing" in origin:
            ctx.run("git remote add upstream git@github.com:pawamoy/pawamoy-testing", silent=True, nofail=True)
            ctx.run(
