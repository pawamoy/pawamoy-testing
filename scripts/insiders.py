...
def feature_list(goals: list[Goal]) -> list[Feature]:
...
numbers = load_json(f"{data_url}/numbers.json")
sponsors = load_json(f"{data_url}/sponsors.json")
...
completed_features = sorted((ft for ft in all_features if ft.since), key=lambda ft: ft.since, reverse=True)


def print_features(features: list[Feature]):
    for feature in features:
        new = ""
        recent = feature.since and datetime.now() - feature.since <= timedelta(days=60)
        if recent:
            date = feature.since.strftime("%B %d, %Y")
            new = f' :material-alert-decagram:{{ .new-feature .vibrate title="Added on {date}" }}'
        print(f"- [x] [{feature.name}]({feature.ref}){new}")


def print_join_sponsors_button():
...
def print_sponsors():
    ...
    for sponsor in sponsors:
        print(f"""<a href="{sponsor['url']}" class="sponsorship-item" title="@{sponsor['name']}"><img src="{sponsor['image']}&size=72"></a>""")
