...
# ISC License
#
# Copyright (c) 2026, Timothee Mazzucotelli and contributors
#
...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `pawamoy_testing`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in pawamoy_testing.__all__ or not hasattr(pawamoy_testing, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in pawamoy_testing.__all__:
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("pawamoy_testing")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and (item.name == "pawamoy_testing" or item.name.startswith("pawamoy_testing."))
        ):
